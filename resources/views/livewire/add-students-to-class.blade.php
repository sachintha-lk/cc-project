<div class="flex items-center justify-center m-5 gap-3">
    <div class="md:w-1/2">
        ...
        <div class="m-3">
            <h2 class="font-semibold text-xl text-gray-700 mb-3">Select Students to add to the
                class {{ $class_name }}</h2>
            <input w wire:model="q" type="search" placeholder="Search" name="student_search"
                   class="block w-1/3 py-2.5 rounded focus:ring-yellow-500 focus:border-yellow-500 sm:text-sm border-gray-400"
                   required>
        </div>
        <div class="m-3">
        ...
        <div>
            <table class="items-center w-full bg-transparent border-collapse">
                <thead>
                <tr>
                    <th class="px-1 bg-gray-50 text-gray-700 align-middle py-3 text-xs font-semibold text-left uppercase border-l-0 border-r-0 whitespace-nowrap">
                        Student Id
                    </th>
                    <th class="px-1 bg-gray-50 text-gray-700 align-middle py-3 text-xs font-semibold text-left uppercase border-l-0 border-r-0 whitespace-nowrap">
                        Student Name
                ...
                @if ($students->count() == 0)
                    <tr class="text-gray-500">
                        <td class="border-t-0 px-1 align-middle text-sm font-normal whitespace-nowrap p-4 text-left">No
                            results found
                        </td>
                    </tr>
                @else
                    @foreach($students as $student)
                        <tr class="text-gray-500">
                            ...
                            <td class="border-t-0 px-1 align-middle text-xs font-medium text-gray-900 whitespace-nowrap py-1 flex">
                                ...
                                {{--                            </div>--}}
                                <div class="ml-2">
                                    <button wire:click="selectStudent({{$student->id}})" wire:loading.attr="disabled"
                                            class="bg-yellow-800 border border-transparent hover:bg-yellow-600 focus:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                                        Select
                                    </button>
    ...
    <div class="md:w-1/3">
        ...
        <div class="flex gap-2 my-4">
            <button wire:click="addSelectedStudentsToClass" wire:loading.attr="disabled"
                    class="bg-yellow-800 border border-transparent  hover:bg-yellow-500 focus:bg-yellow-700 text-white font-bold py-2 px-4 rounded">
                Add Selected Students to Class
            </button>

            <button wire:click="clearSelectedStudents" wire:loading.attr="disabled"
                    class="bg-red-600 border border-transparent  hover:bg-red-500 focus:bg-red-700 text-white font-bold py-2 px-4 rounded">
        ...
        <div>
            <table class="items-center w-full bg-transparent border-collapse">
                ...
                <tr>
                    ...
                    <th class="px-1 bg-gray-50 text-gray-700 align-middle py-3 text-xs font-semibold text-left uppercase border-l-0 border-r-0 whitespace-nowrap min-w-140-px">
                        Remove
                    </th>
                </tr>
                </thead>
                    ...
                    @foreach($selectedStudents as $selectedStudent)
                        <tr class="text-gray-500">
                            ...
                            <td class="border-t-0 px-1 align-middle text-xs font-medium text-gray-900 whitespace-nowrap py-1 flex">
                                ...
                                <div class="ml-2">
                                    <button wire:click="removeFromSelectedStudents({{ $selectedStudent->id }})"
